...
(defcustom scala-indent-offset 2
  "Default indentation offset for Scala indent mod."
  :group 'scala
  :type 'integer
  :safe 'integerp)
...
(defun scalai-mode:indent-scaladoc-asterisk (&optional insert-space-p)
  (message "scalai-mode:indent-scaladoc-asterisk has been deprecated"))
...
;;;
(defun scalai-indent:shift-left (start end &optional count)
  "Shift lines contained in region START END by COUNT columns to the left.
COUNT defaults to `scala-indent-offset'.  If region isn't
...
any lines in the region are indented less than COUNT columns."
  ...
  (if count
    ...
    (setq count scala-indent-offset))
  (when (> count 0)
...
(defun scalai-indent:shift-right (start end &optional count)
...
(defun scalai-indent:dedent-line-backspace (arg)
  "De-indent current line.
Argument ARG is passed to `backward-delete-char-untabify' when
point is not in between the indentation."
  (interactive "*p")
  (unless (scalai-indent:dedent-line)
    (backward-delete-char-untabify arg)))

(put 'scalaiindent:dedent-line-backspace 'delete-selection 'supersede)

(defun scalai-syntax:comment-or-string-p ()
    "Python equiv. Always return nil."
    nil)

(defun scalai-indent:dedent-line ()
  "De-indent current line."
  (interactive "*")
  (when (and (not (bolp))
